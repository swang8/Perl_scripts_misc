...
sub sum_Z
{
	...
	my ($mean, $st) = calculate_genome_wide_Mean_Sigma($haplotype_extent, $pairs_ref, $marker_dist_ref, $mapped_markers_ref);
	my $sum_z = 0;
	foreach (@$pairs_ref)
	{
...
sub pairwise_haplotype_extent_calc
{
	...
	foreach my $blk (@blocks)
	{
		...
		#print $start, "\t", $end, "\n";
		foreach ($start .. $end){$marker_blocks->{$chr}{$_}{$pair} = $blk}
