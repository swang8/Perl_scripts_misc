...
#treefile<-"mytree.phy"
treefile <-"divers_tree.phy2"
treefile <- args[1]
#### classes file - should be tab-delimited - specifies category for each gene
#### format: gene name, class name
classesfile<-"groups.dat2"
classesfile <- args[2]

#### colors file - tab-delimited - specifies a color for each category
#### format: class name, color name (see traskdata/help/R/Rcolors.txt  for list)
colorsfile<-"groups.color"
colorsfile <- args[3]

#### if desired, a list of genes for which we want the branch widths thicker.
#### I was using this to give intact genes thicker branches than  pseudogenes
#### simply a list of genes, one per line
#### if you don't care about this, can give a fake file name or comment it out
...
useintactlist <- "no"  ## "yes" or "no"
regularthickness <- 2
thickerthickness <- 2.5
...
outfile<-paste(treefile,"tree.ps",sep=".")
outfilepdf<-paste(treefile,"tree.pdf",sep=".")
...
pdf(outfilepdf, width=20, height=20)
myplot <- plot.phylo(newtree, edge.color=edgecolors,
                   edge.width=edgewidths,tip.color=tipcolors,
                   label.offset=0.1 ,font=3, main="", cex=0.8,underscore=T,
                   no.margin=T, cex.main=0.1,align.tip.label=T,x.lim=c(-1.1*max(branchlengths), max(branchlengths)*1.5),
...
### if something went wrong, I might want to examine all those objects to
### figure out why, so save everything, as the code runs slowly on big trees
#save.image(file="colortree_usa.Rdata")
