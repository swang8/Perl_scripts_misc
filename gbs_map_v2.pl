...
my $f = shift or die "perl $0 <input>\n";
if ($f){
  my %status = run_the_job($f);
  map{print $_, "\t", $status{$_}, "\n" }keys %status;
}
## 
sub run_the_job{
...
sub parse_blat_output {
  ...
  foreach my $tag (@tags){
    my @res = map{"NA"}1..14;
    if (exists $h{$tag}){
      # map{$h{$tag}[$_] = ",,," unless  $h{$tag}[$_]}0..$#files;
      # CSS: 0
      my @index = (0);
      my $blat_result = "";
      my @blat_results = sort{my @pa=split /,/, $a; my @pb=split /,/,$b; $pb[1] <=> $pa[1] or $pb[2] <=> $pa[2]}@{$h{$tag}}[@index] if @{$h{$tag}}[@index];
      ...
      # nrgene: 1-7
      @index=(1..7);
      @blat_results = grep{defined }@{$h{$tag}}[@index];
      @blat_results = sort{my @pa=split /,/, $a; my @pb=split /,/,$b; $pb[1] <=> $pa[1] or $pb[2] <=> $pa[2]}@blat_results;
      $blat_result = $blat_results[0] || "";
      if($blat_result=~/\S/){
