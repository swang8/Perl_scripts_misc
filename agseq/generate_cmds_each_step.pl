...
foreach my $f (@arr) {
  ...
  if(exists $h{$f2}) {
    ...
    my $cmd="java -jar $Bin/Trimmomatic-0.39/trimmomatic-0.39.jar PE -threads 4 $f $f2 $qc_folder/${acc}_F.fq.gz  $qc_folder/${acc}_FU.fq.gz $qc_folder/${acc}_R.fq.gz $qc_folder/${acc}_RU.fq.gz ILLUMINACLIP:\$EBROOTTRIMMOMATIC/adapters/TruSeq3-PE.fa:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36";
    push @params, $cmd;
  }
  else{
    #push @params, "-se $f 1 5"
     my $cmd="java -jar $Bin/Trimmomatic-0.39/trimmomatic-0.39.jar SE -threads 4 $f  $qc_folder/${acc}_F.fq.gz  ILLUMINACLIP:\$EBROOTTRIMMOMATIC/adapters/TruSeq3-PE.fa:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36";
     push @params, $cmd;
  }   
}
##
...
$aln_pl = "$Bin/align.pl" unless defined $aln_pl;
...
$call_pl = "$Bin/haplotypecaller.pl" unless $call_pl;
my $call_bsub = "3.call.cmds";
