...
for i in $* ; do
    ...
    perl -ne 'BEGIN{$f=$ARGV[0]}chomp;  @t=split /\s+/,$_; next if exists $h{$t[0]}; next unless $t[-1] > 50; $spe=$1 if /^\S+\s+(\S+\s+\S+)/; if ($spe=~/PREDICTED/){if(/PREDICTED:\s+(\S+\s+\S+)/){$spe=$1} }; $h{$t[0]}=1; $g{$spe}+= 1;END{ $tot=0; map{$tot += $_}values %g; @arr=sort{$g{$b} <=> $g{$a}}keys %g; @ps=map{$_. "_" . sprintf("%.2f", $g{$_}/$tot)}@arr[0..2];    print $f, "\t", join(";", @ps), "\n"; map{print STDERR $_, "\t", $g{$_}, "\n"}@arr[0..2]}' ${i}_rand${SAMPLE_SIZE}.blast.out >${i}_rand${SAMPLE_SIZE}.blast.out.top3
