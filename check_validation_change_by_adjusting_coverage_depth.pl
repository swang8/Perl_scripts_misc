...
foreach my $dp (@depth)
{
	...
	# extract snps
	my $snp_file = "min".$dp."_1_2.snps";
	my $cmd_ex_snps = q"perl -ne '@t=split/\t/,$_; next unless @t>=5; $c=0; foreach $r(@t[2,3,4]){if($r=~/^\S{1}_(\d+)/){$c++ if $1>=1}} print $_ if $c>=1' " . $out_file . ">". $snp_file;
	die "$cmd_ex_snps failed \n" if system($cmd_ex_snps);
	
	# extract flanking seq for snps
	my $flank_seq_file = "min".$dp."_1_2_50bp_flanking.fasta";
	my $oligo_file = "min".$dp."_1_2_50bp_oligo.fasta";
	...
	# run blat
	my $blat_out = '9k_blat_min'.$dp."_1_2.out";
...
sub count
{
	...
	my $temp_out = "min".$dp.'_1_2_incommon_info';
