...
my ($input, $len) = @ARGV;
unless($input){print "perl $0 <input_file> <Min_distance, smaller than this means conflict>\n"; exit}
$len += 1; 
# my $file = get_file($q);
my $file = $input;
if ($file){
  ...
  if(scalar @status == 0){print "Good Job! No conflict barcodes detected!\n"}
...
sub create_tmp_file {
   ...
   print STDERR "Creating tmp file: ", $file, "\n";
   open(my $OUT, ">$file") or die "Can not write to file $file:  $!";
   print $OUT $s, "\n";
   close $OUT;
   print STDERR $file, "\n";
   return $file;
