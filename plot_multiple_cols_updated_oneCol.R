...
ldhat_file = "/home/DNA/share_files/WEC_updates/ldhat_w2000_s1000_v2.out_normalized_3bls_min10k_masked"
...
for (chr in CHRS){
 ...
 print(paste("chr", chr, max_len, sep=" "))
 total_len = scale_data[grepl(chr, scale_data$Chr), 4];
 fac = total_len / (max_len/1000000)
 print(paste("Factor ", fac, sep=""))
 ldhat[grepl(chr, ldhat$Chr), 2] = ldhat[grepl(chr, ldhat$Chr), 2] * fac
}
...
for (n in 1:7){
  ...
  layout(matrix(c(1:6, 0), 7, 1, byrow = FALSE), heights=c(1,0.5,1,0.5,1,0.5,0.2))
  par(mar=c(0,4.1,1,1))
...
for (abd in c("a", "b", "d"))
{
  ...
  { 
      ...
      if(chr == "4as"){ldhat_subset$Start_Pos = (abs(ldhat_subset$Start_Pos - xmax))}
      if(chr == "4al"){
        subset_4as <- subset(ldhat, grepl("4as", Chr))
        end_4as = max(subset_4as$Start_Pos) - min(subset_4as$Start_Pos)
        ldhat_subset$Start_Pos = ldhat_subset$Start_Pos + (end_4as+5000000)
      }
      ...
      max_x = max(ldhat$Start_Pos[grepl(n, ldhat$Chr)], na.rm=T)
      min_x = min(ldhat$Start_Pos[grepl(n, ldhat$Chr)], na.rm=T)
      
      if(sl == "s"){
        plot(ldhat_subset$Start_Pos, ldhat_subset$Norm_pi, xlab="", axes=F, ylab="Pi", type="l", col="white", ylim=c(0, limy_pi), xlim=c(0, max_x))
      ...
      if(chr == "4as"){pi_2.5per$Start_Pos = abs(pi_2.5per$Start_Pos - 532e6); pi_97.5per$Start_Pos=abs(pi_97.5per$Start_Pos-532e6)}
      if(chr == "4al"){pi_2.5per$Start_Pos = pi_2.5per$Start_Pos + 185e6; pi_97.5per$Start_Pos=pi_97.5per$Start_Pos+185e6}
      ...
      if(chr == "4as"){rho_97.5per$Start_Pos=abs(rho_97.5per$Start_Pos - 532e6)}
      if(chr == "4al"){rho_97.5per$Start_Pos=rho_97.5per$Start_Pos + 185e6}
  ...
  { 
    ...
    if(sl == "s"){
        plot(ldhat_subset$Start_Pos, ldhat_subset$tajimaD, xlab="", axes=F, ylab="D", type="l", col="white", ylim=c(limy_min_td, limy_max_td), xlim=c(0, max_x))
    ...
    if(chr == "4as"){td_2.5per$Start_Pos = abs(td_2.5per$Start_Pos - 532e6); td_97.5per$Start_Pos=abs(td_97.5per$Start_Pos-532e6)}
    if(chr == "4al"){td_2.5per$Start_Pos = td_2.5per$Start_Pos + 185e6; td_97.5per$Start_Pos=td_97.5per$Start_Pos+185e6}
